...
pub fn create_box_spot_floor(world: &mut World, position: Position) {
    world
        .create_entity()
        .with(Position { z: 5, ..position })
        .with(Renderable::new_static("/images/box_spot_floor.png".to_string()))
        .build();
}
...
pub fn create_box(world: &mut World, position: Position, colour: BoxColour) {
    world
        .create_entity()
        .with(Position { z: 10, ..position })
        .with(Renderable::new_static(format!("/images/box_{}.png", colour)))
        .with(Box { colour })
        .with(Movable)
        .build();
}
